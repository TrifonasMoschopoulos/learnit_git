public class DefaultProductManagementService implements ProductManagementService{
    ...
    public Product getProductById(int productIdToAddToCart) {
        if (idMatchesToProduct(productIdToAddToCart))
            return products[productIdToAddToCart-1];

        return null;
    }

    private boolean idMatchesToProduct(int idToTestIfMatches){
        if (idToTestIfMatches >= 1  && idToTestIfMatches <= getProducts().length){
            return true;
        }
        return false;
    }
    ...
    public String toString(){
        ...
        sb.append(products[products.length - 1].toString());
        return sb.toString();
    }
}
